...
**You can pass an Arc to that interner to anything that expects an [`Interner`]!**
While the interning methods on [`Interner`] require a `&mut self` to also work for single-threaded interners, both
[`Resolver`] and [`Interner`] will be implemented for `Arc::new(interner)` if `interner` is multi-threaded:
...
///
/// Note that you can use `Arc<MultiThreadTokenInterner>` to access interning methods through a shared reference, as
/// well as construct new syntax trees. See [the module documentation](self) for more information and examples.
#[cfg(feature = "multi_threaded_interning")]
